...
from zotero_cli_agent.core.pdf_extractor import MinerUExtractor, get_extractor
from zotero_cli_agent.core.rag import convert_pdfs_to_text
from zotero_cli_agent.core.reader import ZoteroReader
from zotero_cli_agent.core.writer import SYNC_REMINDER, ZoteroWriter
...
DEFAULT_TEMPLATE_DIR = Path(__file__).resolve().parents[2] / "tools" / "note-templates"
AI_NOTE_CONFIG = load_ai_note_config()
DEFAULT_MODEL = AI_NOTE_CONFIG.model or "gpt-5.4"
...
def sanitize_note_markdown(markdown: str, title: str) -> str:
    expected_heading = f"# {NOTE_TITLE_PREFIX}{title}".strip()
    ...
    for index, line in enumerate(lines):
        if line.strip().startswith(f"# {NOTE_TITLE_PREFIX}"):
            return "\n".join(lines[index:]).strip()

    # If the model omitted the heading entirely, add the canonical heading ourselves.
...
def build_openai_client(api_key_env: str, base_url: str | None, timeout: float) -> OpenAI:
    env_key = os.environ.get(api_key_env) if api_key_env else ""
    # Wrapper-provided keys should win so explicit CLIProxyAPI runs are not shadowed by stale config values.
    api_key = env_key or AI_NOTE_CONFIG.api_key
    if not api_key:
...
def analyze_paper(
    ...
    *,
    ...
    return sanitize_note_markdown(text, item.title or item.key)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--analysis-max-tokens", type=int, default=32000, help="Max output tokens for final note analysis.")
