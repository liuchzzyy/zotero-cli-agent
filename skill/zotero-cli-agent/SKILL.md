...
| AI metadata cleanup loop | `zot --json --detail full summarize-all --exclude-tag workflow/metadata_cleaned --exclude-tag update/metadata` + `zot update --from-jsonl updates.jsonl --add-tag workflow/metadata_cleaned` | Export only unprocessed items, clean externally, batch write back and mark processed |
...
zot --json --detail full summarize-all --exclude-tag workflow/metadata_cleaned --exclude-tag update/metadata --limit 100
zot update --from-jsonl cleaned-metadata.jsonl --add-tag workflow/metadata_cleaned
