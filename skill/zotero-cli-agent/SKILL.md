...
| AI metadata cleanup loop | `zot --json --detail full summarize-all --exclude-tag workflow/metadata --exclude-tag update/metadata` + `zot update --from-jsonl updates.jsonl --add-tag workflow/metadata` | Export only unprocessed items, clean externally, batch write back and mark processed |
...
zot --json --detail full summarize-all --exclude-tag workflow/metadata --exclude-tag update/metadata --limit 100
