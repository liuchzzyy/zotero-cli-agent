...
class TestExtractDoi:
    def test_extract_doi_found(self, tmp_path):
        with patch("zotero_cli_agents.core.pdf_extractor.PyMuPdfExtractor.extract_text") as mock_extract:
...
class TestAddPdfMCP:
    def test_handle_add_from_pdf_with_doi_override(self):
        from zotero_cli_agents.mcp_server import _handle_add_from_pdf

        with (
            patch("zotero_cli_agents.mcp_server._get_writer") as mock_get,
            patch("zotero_cli_agents.core.metadata_resolver.resolve_doi", return_value={"title": "T"}),
        ):
    ...
    def test_handle_add_from_pdf_no_doi_found(self):
        ...
        with (
            ...
            patch("zotero_cli_agents.core.pdf_extractor.PyMuPdfExtractor.extract_doi", return_value=None),
        ):
    ...
    def test_handle_add_from_pdf_upload_fails(self):
        ...
        with (
            patch("zotero_cli_agents.mcp_server._get_writer") as mock_get,
            patch("zotero_cli_agents.core.pdf_extractor.PyMuPdfExtractor.extract_doi", return_value="10.1234/test"),
            patch("zotero_cli_agents.core.metadata_resolver.resolve_doi", return_value=None),
        ):
            mock_writer = MagicMock()
            mock_get.return_value = mock_writer
            mock_writer.add_item.return_value = "NEW001"
